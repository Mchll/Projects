...
abstract class TweetSet {
  ...
  def trending(words : List[String]) : TweetList = {
    this.filter(tw => words.exists(word => tw.text.contains(word))).descendingByRetweet
  }
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (right.isEmpty)
      ...
      max(right.mostRetweeted, elem)
    else
      max(right.mostRetweeted, max(left.mostRetweeted, elem))
  }

  def descendingByRetweet: TweetList = {
    val asc = this.mostRetweeted
    new Cons(asc, this.remove(asc).descendingByRetweet)
  }
  /**
    * The following methods are already implemented
    */
